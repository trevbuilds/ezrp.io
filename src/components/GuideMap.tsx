...
const DOMAIN_X = [-560, -280, 0, 280, 560];
...
function useLayout() {
  return useMemo(() => {
    ...
    allBusinessDomains.forEach((domain, domainIndex) => {
      ...
      const domainX = DOMAIN_X[domainIndex] ?? 0;
      nodes.set(domainId, { id: domainId, label: domain, x: domainX, y: -270, kind: "domain", domain });
      links.push({ from: "core", to: domainId });

      const streams = streamsInDomain(domain);
      streams.forEach((stream, streamIndex) => {
        const streamId = `stream:${stream}`;
        const streamX = domainX + (streamIndex - (streams.length - 1) / 2) * 120;
        nodes.set(streamId, {
          id: streamId,
          label: stream,
          x: streamX,
        ...
        const streamGuides = guidesInStream(stream).filter(
          (guide) => guide.slug !== "customer-relationship-management" &&
            guide.slug !== "financial-accounting" &&
            guide.slug !== "human-capital-management" &&
            guide.slug !== "enterprise-asset-management",
        );
        streamGuides.forEach((guide, guideIndex) => {
          const columns = Math.min(4, Math.max(1, streamGuides.length));
          const column = guideIndex % columns;
          const row = Math.floor(guideIndex / columns);
          const guideX = streamX + (column - (columns - 1) / 2) * 68;
    ...
    CROSS_CUTTING.forEach((slug, index) => {
      ...
      allBusinessDomains.forEach((domain) => {
        links.push({ from: id, to: `domain:${domain}`, cross: true });
      });
    });
...
export function GuideMap() {
  ...
  const related = useMemo(() => {
    ...
    if (node.kind === "domain") {
      streamsInDomain(node.domain as BusinessDomain).forEach((stream) => set.add(`stream:${stream}`));
    }
    if (node.kind === "stream") {
  ...
  const nodeMark = (node: MapNode) => {
    const radius = node.kind === "domain" ? 13 : node.kind === "stream" ? 9 : node.kind === "cross" ? 8 : 5;
  ...
  return (
    <div className="relative">
          ...
          <motion.div drag dragMomentum={false} className="size-full">
            ...
            >
                ...
                {links.map((link) => {
                  ...
                  return (
                    <path
                      ...
                      d={linkPath(link.from === "core" ? { id: "core", label: "EZRP", x: 0, y: -405, kind: "domain" } : nodes.get(link.from), nodes.get(link.to))}
              ...
              <g className="guide-brain__core">
                ...
                <text x={0} y={-400} textAnchor="middle">EZRP</text>
              </g>

              <text x={-720} y={-305} className="fill-muted-foreground font-mono text-[11px] uppercase">Business domains</text>
              <text x={-720} y={-70} className="fill-muted-foreground font-mono text-[11px] uppercase">Value streams</text>
              <text x={-720} y={130} className="fill-muted-foreground font-mono text-[11px] uppercase">Guide topics</text>
              <line x1={-720} y1={605} x2={720} y2={605} className="stroke-border" strokeDasharray="4 8" />
              <text x={-720} y={635} className="fill-primary font-mono text-[11px] uppercase">Cross-cutting capabilities · linked across every domain</text>
              ...
              <g className="guide-brain__nodes">
                {[...nodes.values()].map((node) => {
                  ...
                  if (node.kind === "domain") {
                    return <Link key={node.id} to="/guides" search={{ domain: node.domain }} className={className} {...handlers}>{content}</Link>;
                  }
                  if (node.kind === "stream") {
                    return <Link key={node.id} to="/guides" search={{ stream: node.stream }} className={className} {...handlers}>{content}</Link>;
                  }
                  if (node.kind === "cross" && node.guide) {
                    return <Link key={node.id} to="/guides" search={{ module: node.guide.slug }} className={className} {...handlers}>{content}</Link>;
                  }
                  if (node.guide) {
                    return <Link key={node.id} to="/guides/$slug" params={{ slug: node.guide.slug }} className={className} {...handlers}>{content}</Link>;
                  }
                  return null;
                })}
              </g>
      ...
      <div className="absolute right-3 top-3 flex flex-col gap-1">
        <Button variant="outline" size="icon" onClick={() => setZoom((value) => Math.min(1.55, value + 0.12))} aria-label="Zoom in"><Plus /></Button>
        <Button variant="outline" size="icon" onClick={() => setZoom((value) => Math.max(0.55, value - 0.12))} aria-label="Zoom out"><Minus /></Button>
        <Button variant="outline" size="icon" onClick={() => setZoom(0.9)} aria-label="Reset zoom"><Crosshair /></Button>
      </div>

      <div className="panel mt-3 rounded-lg p-4">
        {selected ? (
          <div>
            <p className="label-xs">{selected.kind === "cross" ? "Cross-cutting capability" : selected.kind}</p>
          ...
          <p className="text-sm text-muted-foreground">
            EZRP is the core. Follow a business domain into its value streams and guide topics; shared capabilities connect across every domain.
          </p>
        )}
      </div>
    </div>
